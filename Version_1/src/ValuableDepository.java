...
public class ValuableDepository<Valuable> implements Deposit<Valuable>
{
    ...
    {
        arrayList = new ArrayList<Valuable>();
        this.size = size;
        log = Logger.getInstance();
    }

    @Override
    public synchronized void enqueue(Valuable valuable)
    {
    ...
    @Override
    public synchronized Valuable dequeue()
    {
        if (arrayList.size() > 0)
        {
    ...
    @Override
    public synchronized Valuable getLastValue()
    {
        return arrayList.get(arrayList.size() - 1);
    }

    @Override
    public synchronized Valuable getValuableAtIndex(int index) {
        if(arrayList.size() > 0){
